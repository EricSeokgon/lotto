"""SPEC-LOTTO-148: 13의 배수 분포 분석 테스트."""
...
def test_required_keys(monkeypatch):
    ...
    for key in ("total", "mult13_count", "avg", "expected", "diff", "best_count",
                "best_count_pct", "zero_pct", "dist_list", "freq_list", "recent"):
        assert key in result


def test_mult13_count_is_3(monkeypatch):
    monkeypatch.setattr(wd, "get_draws", lambda: SAMPLE_DRAWS)
    result = wd.get_multiples13_analysis()
    assert result is not None
    assert result["mult13_count"] == 3
...
def test_dist_list_length_is_4(monkeypatch):
    monkeypatch.setattr(wd, "get_draws", lambda: SAMPLE_DRAWS)
    result = wd.get_multiples13_analysis()
    assert result is not None
    assert len(result["dist_list"]) == 4


def test_freq_list_length_is_3(monkeypatch):
    monkeypatch.setattr(wd, "get_draws", lambda: SAMPLE_DRAWS)
    result = wd.get_multiples13_analysis()
    assert result is not None
...
def test_multiples13_page_200():
    response = client.get("/stats/multiples-13")
