"""SPEC-LOTTO-151: 23의 배수 분포 분석 테스트."""
...
def test_required_keys(monkeypatch):
    ...
    for key in ("total", "mult23_count", "avg", "expected", "diff", "best_count",
                "best_count_pct", "zero_pct", "dist_list", "freq_list", "recent"):
        assert key in result


def test_mult23_count_is_1(monkeypatch):
    monkeypatch.setattr(wd, "get_draws", lambda: SAMPLE_DRAWS)
    result = wd.get_multiples23_analysis()
    assert result is not None
    assert result["mult23_count"] == 1
...
def test_multiples23_page_200():
    response = client.get("/stats/multiples-23")
