"""SPEC-LOTTO-149: 17의 배수 분포 분석 테스트."""
...
def test_required_keys(monkeypatch):
    ...
    for key in ("total", "mult17_count", "avg", "expected", "diff", "best_count",
                "best_count_pct", "zero_pct", "dist_list", "freq_list", "recent"):
        assert key in result


def test_mult17_count_is_2(monkeypatch):
    monkeypatch.setattr(wd, "get_draws", lambda: SAMPLE_DRAWS)
    result = wd.get_multiples17_analysis()
    assert result is not None
    assert result["mult17_count"] == 2
...
def test_multiples17_page_200():
    response = client.get("/stats/multiples-17")
