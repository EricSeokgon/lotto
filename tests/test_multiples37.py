"""SPEC-LOTTO-154: 37의 배수 분포 분석 테스트."""
...
def test_required_keys(monkeypatch):
    ...
    for key in ("total", "mult37_count", "avg", "expected", "diff", "best_count",
                "best_count_pct", "zero_pct", "dist_list", "freq_list", "recent"):
        assert key in result


def test_mult37_count_is_1(monkeypatch):
    monkeypatch.setattr(wd, "get_draws", lambda: SAMPLE_DRAWS)
    result = wd.get_multiples37_analysis()
    assert result is not None
    assert result["mult37_count"] == 1
...
def test_multiples37_page_200():
    response = client.get("/stats/multiples-37")
