"""SPEC-LOTTO-167: 고구간(31~45) 분포 분석 테스트."""
...
def test_zone_list_is_31_to_45(monkeypatch):
    monkeypatch.setattr(wd, "get_draws", lambda: SAMPLE_DRAWS)
    result = wd.get_high_zone_analysis()
    assert result is not None
    assert result["zone_list"] == list(range(31, 46))
...
def test_high_zone_page_200():
    response = client.get("/stats/high-zone")
