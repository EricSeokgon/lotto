...
async def get_range_dist_stats_route() -> dict[str, Any]:
    """본번호 6개의 구간별 분포 통계를 반환합니다 (SPEC-LOTTO-068).

    - 본번호 6개(보너스 제외)를 5개 고정 구간("1-9","10-19","20-29",
      "30-39","40-45")으로 분류해 구간별 total_count/draw_count/avg_per_draw/
      pct_of_numbers/draw_pct 와 most_covered_range 를 제공한다.
    - range_stats 는 5개 구간 키를 항상 포함한다(미관측 구간 0 유지).
    - 데이터 부재 시에도 200 으로 정상 응답 (total_draws=0).
    """
...
async def get_mult5_stats_route() -> dict[str, Any]:
    """본번호 6개 중 5의 배수 포함 개수(0~6) 분포 통계를 반환합니다 (SPEC-LOTTO-075).

    - 회차별 본번호 6개(보너스 제외)에서 5로 나누어 떨어지는 번호 개수를 산출하고,
      전체 회차를 "0".."6" 7개 키로 분류한다.
    - avg_mult5_count / most_common_count(동률 시 작은 값) / high_mult5_pct(>=3 비율)
      / mult5_distribution 을 제공한다.
    - mult5_distribution 은 7개 키를 항상 포함한다(미관측 0 유지).
    - SPEC-073(3의 배수)·SPEC-074(짝수 개수)와 독립적인 별도 엔드포인트다.
    - 데이터 부재 시에도 200 으로 정상 응답 (total_draws=0).
    """
    return wd.get_mult5_stats(wd.get_draws())


@router.get("/stats/mult4")
async def get_mult4_stats_route() -> dict[str, Any]:
    """본번호 6개 중 4의 배수 포함 개수(0~6) 분포 통계를 반환합니다 (SPEC-LOTTO-076).

    - 회차별 본번호 6개(보너스 제외)에서 4로 나누어 떨어지는 번호 개수를 산출하고,
      전체 회차를 "0".."6" 7개 키로 분류한다.
    - avg_mult4_count / most_common_count(동률 시 작은 값) / high_mult4_pct(>=3 비율)
      / mult4_distribution 을 제공한다.
    - mult4_distribution 은 7개 키를 항상 포함한다(미관측 0 유지).
    - SPEC-073(3의 배수)·SPEC-074(짝수)·SPEC-075(5의 배수)와 독립적인 별도 엔드포인트다.
    - 데이터 부재 시에도 200 으로 정상 응답 (total_draws=0).
    """
...
@router.get("/stats/max_gap_dist")
async def get_max_gap_dist_stats_route() -> dict[str, Any]:
    """본번호 6개 인접 간격 최댓값(max_gap)의 구간별 분포 통계를 반환합니다 (SPEC-LOTTO-080).

    - 회차별 정렬 본번호 6개(보너스 제외)의 인접 차이 5개 중 최댓값을 산출하고,
      6개 고정 구간("1-5","6-10","11-15","16-20","21-30","31+")으로 분류한다.
    - avg_max_gap(평균) / most_common_range(동률 시 앞선 구간)
      / high_gap_pct(max_gap>=21 비율) / max_gap_distribution 을 제공한다.
    - max_gap_distribution 은 6개 키를 항상 포함한다(미관측 0 유지).
    - SPEC-056(get_gap_stats, small/medium/large)과는 출력 구조가 다른 별개 엔드포인트다.
    - 데이터 부재 시에도 200 으로 정상 응답 (total_draws=0).
    """
    return wd.get_max_gap_dist_stats(wd.get_draws())


@router.get("/stats/even_run")
async def get_even_run_stats_route() -> dict[str, Any]:
    """본번호 6개 짝수 연속 묶음(간격=2) 수(0~3)의 분포 통계를 반환합니다 (SPEC-LOTTO-081).

    - 회차별 본번호 6개(보너스 제외) 중 짝수만 추출하여, 간격이 정확히 2인 연속
      짝수 묶음(길이>=2)의 수를 산출하고, 4개 고정 키("0","1","2","3")로 분류한다.
    - has_even_run_pct(>=1 비율) / most_common_group_count(동률 시 작은 값)
      / avg_even_run_count(회차당 평균 묶음 수) / even_run_distribution 을 제공한다.
    - even_run_distribution 은 4개 키를 항상 포함한다(미관측 0 유지).
    - SPEC-074(짝수 총 개수)·SPEC-069(연속 쌍, 간격1)와는 독립적인 별개 엔드포인트다.
    - 데이터 부재 시에도 200 으로 정상 응답 (total_draws=0).
    """
...
@router.get("/stats/last_digit_pair")
async def get_last_digit_pair_stats_route() -> dict[str, Any]:
    """본번호 6개의 일의 자리 중복 그룹 수(0~3)의 분포 통계를 반환합니다 (SPEC-LOTTO-085).

    - 회차별 본번호 6개(보너스 제외)를 일의 자리(n % 10)별로 묶어, 같은 일의 자리를
      2개 이상 가진 그룹의 수를 산출하고 4개 고정 키("0"~"3")로 분류한다.
    - avg_pair_count(평균) / most_common_pair_count(동률 시 작은 값)
      / has_pair_pct(중복 그룹 수>=1 비율) / last_digit_pair_distribution 을 제공한다.
    - last_digit_pair_distribution 은 4개 키를 항상 포함한다(미관측 0 유지).
    - SPEC-063/079(끝자리 합계 분포)와는 정의·출력 구조가 다른 독립적인 별개 엔드포인트다.
    - 데이터 부재 시에도 200 으로 정상 응답 (total_draws=0).
    """
...
# @MX:NOTE: [AUTO] SPEC-LOTTO-087 — 본번호 중앙값 10단위 구간(5개) 분포 통계 API
# @MX:SPEC: SPEC-LOTTO-087
@router.get("/stats/median_range")
async def get_median_range_stats_endpoint() -> dict[str, Any]:
    """본번호 중앙값(3·4번째 평균)의 10단위 구간 5개 분포 통계를 반환합니다 (SPEC-LOTTO-087).

    - 구간: "1-9","10-19","20-29","30-39","40-45" (중앙값 기준 10단위).
    - avg_median / most_common_range(동률 시 앞선 구간) / central_median_pct("20-29" 비율)
      / median_range_distribution 을 제공한다.
    - median_range_distribution 은 5개 키를 항상 포함한다(미관측 0 유지).
    - SPEC-071(중앙값 9구간)과는 버킷 정의가 다른 별개 엔드포인트다.
    - 데이터 부재 시에도 200 으로 정상 응답 (total_draws=0).
    """
    return wd.get_median_range_stats(wd.get_draws())


# @MX:NOTE: [AUTO] SPEC-LOTTO-088 — 본번호 간격 분산(균등도) 5개 구간 분포 통계 API
# @MX:SPEC: SPEC-LOTTO-088
@router.get("/stats/gap_variance")
async def get_gap_variance_stats_endpoint() -> dict[str, Any]:
    """본번호 인접 간격 5개의 모분산(균등도) 구간 5개 분포 통계를 반환합니다 (SPEC-LOTTO-088).

    - 구간: "0-10"(<10),"10-30"(<30),"30-60"(<60),"60-100"(<100),"100+"(>=100).
    - avg_variance / most_common_range(동률 시 앞선 구간) / uniform_gap_pct("0-10" 비율)
      / gap_variance_distribution 을 제공한다.
    - gap_variance_distribution 은 5개 키를 항상 포함한다(미관측 0 유지).
    - SPEC-056(간격 패턴)·SPEC-079(최대 간격 분포)와는 산출 대상이 다른 별개 엔드포인트다.
    - 데이터 부재 시에도 200 으로 정상 응답 (total_draws=0).
    """
...
# @MX:NOTE: [AUTO] SPEC-LOTTO-091 — 소수 이웃 포함 개수 분포 통계 API
# @MX:SPEC: SPEC-LOTTO-091
@router.get("/stats/prime_neighbor")
async def get_prime_neighbor_stats_endpoint() -> dict[str, Any]:
    """본번호 6개 중 소수 이웃 포함 개수(0~6) 분포 통계를 반환합니다 (SPEC-LOTTO-091).

    - 키: "0"~"6" (소수 이웃 집합에 포함된 번호 개수 기준).
    - 소수 이웃이란 1~45에서 자기 자신이 소수이거나 소수와 인접(소수±1)한 번호이다.
    - avg_neighbor_count / most_common_count(동률 시 가장 작은 키)
      / high_neighbor_pct(개수 5 이상 비율) / prime_neighbor_distribution 을 제공한다.
    - prime_neighbor_distribution 은 7개 키를 항상 포함한다(미관측 0 유지).
    - SPEC-058(소수 개수만 세는 지표)와는 출력 구조가 다른 별개 엔드포인트.
    - 데이터 부재 시에도 200 으로 정상 응답 (total_draws=0).
    """
...
async def get_alternation_stats_endpoint() -> dict[str, Any]:
    """정렬된 본번호 인접 쌍의 홀짝 교차 단계("교차0"~"교차5") 분포를 반환합니다 (SPEC-LOTTO-094).

    - 키: "교차0"~"교차5" (정렬된 번호열의 홀짝 교차 횟수 0~5).
    - avg_alternation(평균 교차 횟수) / most_common_level(동률 시 키 순서상 앞선 것)
      / full_alternation_pct("교차5" 비율) / alternation_distribution 을 제공한다.
    - alternation_distribution 은 6개 키를 항상 포함한다(미관측 0 유지).
    - SPEC-084(get_parity_transition_stats: 정수 키 + 전환>=4 비율)와는 출력 구조가
      다른 별개 엔드포인트.
    - 데이터 부재 시에도 200 으로 정상 응답 (total_draws=0).
    """
...
def _collect_worker(full: bool, start_from: int, max_drw_no: int) -> None:
    ...
    # @MX:NOTE: [AUTO] SPEC-LOTTO-111 REQ-PW-005 — 3회 연속 HTML 오류 시 Playwright 폴백
    """
    import time
    ...
    html_error_count = 0  # SPEC-LOTTO-111: HTML 오류 누적 카운트
    playwright_mode = False  # SPEC-LOTTO-111: Playwright 폴백 활성화 여부
    pw_collector: PlaywrightCollector | None = None
    ...
    for idx, drw_no in enumerate(targets, 1):
        ...
        # SPEC-LOTTO-111 REQ-PW-005: Playwright 폴백 모드
        if playwright_mode:
            if pw_collector is None:
                pw_collector = PlaywrightCollector()
            draw = pw_collector.fetch_draw_sync(drw_no)
            time.sleep(0.2)
        else:
            try:
                ...
                # 3회 연속 HTML 오류 시 Playwright 폴백으로 전환
                if html_error_count >= 3:
                    logger.warning(
                        "HTML 응답이 %d회 발생했습니다. Playwright 폴백으로 전환합니다.",
                        html_error_count,
                    )
...
async def upload_excel_draws(file: UploadFile = File(...)) -> dict[str, Any]:  # noqa: B008
    ...
    try:
        from lotto.excel_parser import parse_excel

        parsed = parse_excel(Path(tmp_path))
    except Exception as exc:
        raise HTTPException(status_code=422, detail=f"엑셀 파싱 실패: {exc}") from exc
    finally:
        os.unlink(tmp_path)

    if not parsed:
        raise HTTPException(status_code=422, detail="유효한 회차 데이터를 찾을 수 없습니다.")
    ...
    new_draws = [d for d in parsed if d.drwNo not in existing_set]
    if not new_draws:
        return {
            "added": 0,
            "skipped": len(parsed),
            "message": f"모두 기존 데이터와 중복입니다. ({len(parsed)}회차)",
        }

    merged = sorted(existing + new_draws, key=lambda d: d.drwNo)
    collector.save_csv(merged)
    invalidate_cache()

    return {
        "added": len(new_draws),
        "skipped": len(parsed) - len(new_draws),
        "total": len(merged),
        "message": f"{len(new_draws)}회차 추가됨 (총 {len(merged)}회차)",
        "new_drw_nos": sorted(d.drwNo for d in new_draws),
    }
