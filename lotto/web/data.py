...
def get_composite_analysis() -> dict | None:
    ...
    COMPOSITES = {n for n in range(1, 46) if n > 1 and not _is_prime(n)}  # noqa: N806
    ...
    max_k = 6
    count_dist: dict[int, int] = {k: 0 for k in range(max_k + 1)}  # noqa: C420
    freq: dict[int, int] = {m: 0 for m in COMPOSITES}  # noqa: C420

    for draw in draws:
        nums = set(draw.numbers())
        in_draw = nums & COMPOSITES
        count_dist[len(in_draw)] = count_dist.get(len(in_draw), 0) + 1
        for m in in_draw:
            freq[m] += 1
    ...
    freq_list = sorted(
        [{"number": m, "count": freq[m], "pct": round(freq[m] / total * 100, 1)} for m in COMPOSITES],
        key=lambda x: x["number"],
    )
    ...
    return {
        "total": total,
        "composite_count": len(COMPOSITES),
        "composites_list": sorted(COMPOSITES),
        "avg": avg,
        "expected": expected,
        "diff": round(avg - expected, 3),
        "best_count": best_count,
...
def get_mid_zone_analysis() -> dict | None:
    """SPEC-LOTTO-166: 중구간(16~30) 분포 분석."""
    draws = get_draws()
    if not draws:
        return None

    MID_ZONE = set(range(16, 31))  # noqa: N806

    total = len(draws)
    expected = round(len(MID_ZONE) / 45 * 6, 3)

    max_k = 6
    count_dist: dict[int, int] = {k: 0 for k in range(max_k + 1)}  # noqa: C420
    freq: dict[int, int] = {m: 0 for m in MID_ZONE}  # noqa: C420

    for draw in draws:
        nums = set(draw.numbers())
        in_draw = nums & MID_ZONE
        count_dist[len(in_draw)] = count_dist.get(len(in_draw), 0) + 1
        for m in in_draw:
            freq[m] += 1
    ...
    freq_list = sorted(
        [{"number": m, "count": freq[m], "pct": round(freq[m] / total * 100, 1)} for m in MID_ZONE],
        key=lambda x: x["number"],
    )
    ...
    return {
        "total": total,
        "zone_count": len(MID_ZONE),
        "zone_list": sorted(MID_ZONE),
        "avg": avg,
        "expected": expected,
        "diff": round(avg - expected, 3),
        "best_count": best_count,
...
def get_high_zone_analysis() -> dict | None:
    """SPEC-LOTTO-167: 고구간(31~45) 분포 분석."""
    draws = get_draws()
    if not draws:
        return None

    HIGH_ZONE = set(range(31, 46))  # noqa: N806

    total = len(draws)
    expected = round(len(HIGH_ZONE) / 45 * 6, 3)

    max_k = 6
    count_dist: dict[int, int] = {k: 0 for k in range(max_k + 1)}  # noqa: C420
    freq: dict[int, int] = {m: 0 for m in HIGH_ZONE}  # noqa: C420

    for draw in draws:
        nums = set(draw.numbers())
        in_draw = nums & HIGH_ZONE
        count_dist[len(in_draw)] = count_dist.get(len(in_draw), 0) + 1
        for m in in_draw:
            freq[m] += 1
    ...
    freq_list = sorted(
        [{"number": m, "count": freq[m], "pct": round(freq[m] / total * 100, 1)} for m in HIGH_ZONE],
        key=lambda x: x["number"],
    )
    ...
    return {
        "total": total,
        "zone_count": len(HIGH_ZONE),
        "zone_list": sorted(HIGH_ZONE),
        "avg": avg,
        "expected": expected,
        "diff": round(avg - expected, 3),
        "best_count": best_count,
...
def get_twin_prime_analysis() -> dict | None:
    """SPEC-LOTTO-170: 쌍둥이 소수 포함 분포 분석.

    쌍둥이 소수: (p, p+2) 쌍에서 두 수 모두 소수인 경우.
    1~45 내 해당 번호: {3,5,7,11,13,17,19,29,31,41,43} = 11개.
    """
    draws = get_draws()
    if not draws:
        return None

    # 1~45 내 쌍둥이 소수 구성원 (3,5), (5,7), (11,13), (17,19), (29,31), (41,43)
    TWIN_PRIMES = {3, 5, 7, 11, 13, 17, 19, 29, 31, 41, 43}  # noqa: N806
    total = len(draws)
    expected = round(len(TWIN_PRIMES) / 45 * 6, 3)
    max_k = 6

    count_dist: dict[int, int] = {k: 0 for k in range(max_k + 1)}  # noqa: C420
    freq: dict[int, int] = {m: 0 for m in TWIN_PRIMES}

    for draw in draws:
        nums = set(draw.numbers())
        in_draw = nums & TWIN_PRIMES
        count_dist[len(in_draw)] = count_dist.get(len(in_draw), 0) + 1
        for m in in_draw:
            freq[m] += 1
    ...
    freq_list = sorted(
        [{"number": m, "count": freq[m], "pct": round(freq[m] / total * 100, 1)} for m in TWIN_PRIMES],
        key=lambda x: x["number"],
    )

    # 쌍둥이 소수 쌍 목록 (참고용)
    twin_pairs = [(3, 5), (5, 7), (11, 13), (17, 19), (29, 31), (41, 43)]
...
def get_triangular_analysis() -> dict | None:
    """SPEC-LOTTO-174: 삼각수(Triangular Number) 포함 분포 분석.

    삼각수: T(n) = n(n+1)/2. 1~45 내 9개:
    T1=1, T2=3, T3=6, T4=10, T5=15, T6=21, T7=28, T8=36, T9=45.
    """
    draws = get_draws()
    if not draws:
        return None

    TRIANGULARS = {1, 3, 6, 10, 15, 21, 28, 36, 45}  # noqa: N806
    # T(n) → n 역매핑
    TRIANGULAR_N = {1: 1, 3: 2, 6: 3, 10: 4, 15: 5, 21: 6, 28: 7, 36: 8, 45: 9}  # noqa: N806
    total = len(draws)
    expected = round(len(TRIANGULARS) / 45 * 6, 3)
    max_k = min(6, len(TRIANGULARS))

    count_dist: dict[int, int] = {k: 0 for k in range(max_k + 1)}  # noqa: C420
    freq: dict[int, int] = {m: 0 for m in TRIANGULARS}

    for draw in draws:
        nums = set(draw.numbers())
        in_draw = nums & TRIANGULARS
        cnt = len(in_draw)
        count_dist[cnt] = count_dist.get(cnt, 0) + 1
        for m in in_draw:
            freq[m] += 1
    ...
    return {
        "total": total,
        "triangular_count": len(TRIANGULARS),
        "triangular_list": sorted(TRIANGULARS),
        "avg": avg,
        "expected": expected,
        "diff": round(avg - expected, 3),
        "best_count": best_count,
...
def get_perfect_analysis() -> dict | None:
    """SPEC-LOTTO-175: 완전수(Perfect Number) 포함 분포 분석.

    완전수: 자신을 제외한 양의 약수의 합 = 자신.
    1~45 범위 내 2개: {6, 28}.
    ...
    PERFECTS = {6, 28}  # noqa: N806
    # 완전수 번호 → 약수 역매핑
    PERFECT_DIVISORS = {6: "1+2+3", 28: "1+2+4+7+14"}  # noqa: N806
    total = len(draws)
    expected = round(len(PERFECTS) / 45 * 6, 3)
    max_k = min(6, len(PERFECTS))

    count_dist: dict[int, int] = {k: 0 for k in range(max_k + 1)}  # noqa: C420
    freq: dict[int, int] = {m: 0 for m in PERFECTS}  # noqa: C420

    for draw in draws:
        nums = set(draw.numbers())
        in_draw = nums & PERFECTS
        cnt = len(in_draw)
        count_dist[cnt] = count_dist.get(cnt, 0) + 1
        for m in in_draw:
            freq[m] += 1
    ...
    freq_list = [
        {
            "number": m,
            "divisors": PERFECT_DIVISORS[m],
            "count": freq[m],
            "pct": round(freq[m] / total * 100, 1),
        }
        for m in sorted(PERFECTS)
    ]
...
def get_hexagonal_analysis() -> dict | None:
    """SPEC-LOTTO-177: 육각수(Hexagonal Number) 포함 분포 분석.

    육각수 H(n) = n(2n-1).
    1~45 범위 내 5개: {1, 6, 15, 28, 45}.
    (H(1)=1, H(2)=6, H(3)=15, H(4)=28, H(5)=45)
    이론 기댓값 = 5/45 × 6 ≈ 0.667.
    """
    draws = get_draws()
    if not draws:
    ...
    # H(n) = n(2n-1); 1~45 범위 내 육각수
    HEXAGONS = {1, 6, 15, 28, 45}  # noqa: N806
    HEXAGON_N = {1: 1, 6: 2, 15: 3, 28: 4, 45: 5}  # noqa: N806
    total = len(draws)
    expected = round(len(HEXAGONS) / 45 * 6, 3)
    max_k = min(6, len(HEXAGONS))

    count_dist: dict[int, int] = {k: 0 for k in range(max_k + 1)}  # noqa: C420
    freq: dict[int, int] = {m: 0 for m in HEXAGONS}  # noqa: C420

    for draw in draws:
        nums = set(draw.numbers())
        in_draw = nums & HEXAGONS
        cnt = len(in_draw)
        count_dist[cnt] = count_dist.get(cnt, 0) + 1
        for m in in_draw:
            freq[m] += 1
    ...
    freq_list = [
        {
            "number": m,
            "n": HEXAGON_N[m],
            "formula": f"{HEXAGON_N[m]}×(2×{HEXAGON_N[m]}-1)",
            "count": freq[m],
            "pct": round(freq[m] / total * 100, 1),
        }
        for m in sorted(HEXAGONS)
    ]
...
def get_heptagonal_analysis() -> dict | None:
    """SPEC-LOTTO-178: 칠각수(Heptagonal Number) 포함 분포 분석.

    칠각수 H(n) = n(5n-3)/2.
    1~45 범위 내 4개: {1, 7, 18, 34}.
    (H(1)=1, H(2)=7, H(3)=18, H(4)=34)
    이론 기댓값 = 4/45 × 6 ≈ 0.533.
    """
    draws = get_draws()
    if not draws:
    ...
    # H(n) = n(5n-3)/2; 1~45 범위 내 칠각수
    HEPTAGONS = {1, 7, 18, 34}  # noqa: N806
    HEPTAGON_N = {1: 1, 7: 2, 18: 3, 34: 4}  # noqa: N806
    total = len(draws)
    expected = round(len(HEPTAGONS) / 45 * 6, 3)
    max_k = min(6, len(HEPTAGONS))

    count_dist: dict[int, int] = {k: 0 for k in range(max_k + 1)}  # noqa: C420
    freq: dict[int, int] = {m: 0 for m in HEPTAGONS}  # noqa: C420

    for draw in draws:
        nums = set(draw.numbers())
        in_draw = nums & HEPTAGONS
        cnt = len(in_draw)
        count_dist[cnt] = count_dist.get(cnt, 0) + 1
        for m in in_draw:
            freq[m] += 1
    ...
    freq_list = [
        {
            "number": m,
            "n": HEPTAGON_N[m],
            "formula": f"{HEPTAGON_N[m]}×(5×{HEPTAGON_N[m]}-3)÷2",
            "count": freq[m],
            "pct": round(freq[m] / total * 100, 1),
        }
        for m in sorted(HEPTAGONS)
    ]
    ...
    return {
        "total": total,
        "heptagon_count": len(HEPTAGONS),
        "heptagon_list": sorted(HEPTAGONS),
        "avg": avg,
        "expected": expected,
        "diff": round(avg - expected, 3),
        "best_count": best_count,
...
def get_octagonal_analysis() -> dict | None:
    """SPEC-LOTTO-179: 팔각수(Octagonal Number) 포함 분포 분석.

    팔각수 O(n) = n(3n-2).
    1~45 범위 내 4개: {1, 8, 21, 40}.
    (O(1)=1, O(2)=8, O(3)=21, O(4)=40)
    이론 기댓값 = 4/45 × 6 ≈ 0.533.
    """
    draws = get_draws()
    if not draws:
    ...
    # O(n) = n(3n-2); 1~45 범위 내 팔각수
    OCTAGONS = {1, 8, 21, 40}  # noqa: N806
    OCTAGON_N = {1: 1, 8: 2, 21: 3, 40: 4}  # noqa: N806
    total = len(draws)
    expected = round(len(OCTAGONS) / 45 * 6, 3)
    max_k = min(6, len(OCTAGONS))

    count_dist: dict[int, int] = {k: 0 for k in range(max_k + 1)}  # noqa: C420
    freq: dict[int, int] = {m: 0 for m in OCTAGONS}  # noqa: C420

    for draw in draws:
        nums = set(draw.numbers())
        in_draw = nums & OCTAGONS
        cnt = len(in_draw)
        count_dist[cnt] = count_dist.get(cnt, 0) + 1
        for m in in_draw:
            freq[m] += 1
    ...
    freq_list = [
        {
            "number": m,
            "n": OCTAGON_N[m],
            "formula": f"{OCTAGON_N[m]}×(3×{OCTAGON_N[m]}-2)",
            "count": freq[m],
            "pct": round(freq[m] / total * 100, 1),
        }
        for m in sorted(OCTAGONS)
    ]
    ...
    return {
        "total": total,
        "octagon_count": len(OCTAGONS),
        "octagon_list": sorted(OCTAGONS),
        "avg": avg,
        "expected": expected,
        "diff": round(avg - expected, 3),
        "best_count": best_count,
...
def get_decagonal_analysis() -> dict | None:
    """SPEC-LOTTO-181: 십각수(Decagonal Number) 포함 분포 분석.
    ...
    # D(n) = n(4n-3); 1~45 범위 내 십각수
    DECAGONS = {1, 10, 27}  # noqa: N806
    DECAGON_N = {1: 1, 10: 2, 27: 3}  # noqa: N806
    total = len(draws)
    expected = round(len(DECAGONS) / 45 * 6, 3)
    max_k = min(6, len(DECAGONS))

    count_dist: dict[int, int] = {k: 0 for k in range(max_k + 1)}  # noqa: C420
    freq: dict[int, int] = {m: 0 for m in DECAGONS}  # noqa: C420

    for draw in draws:
        nums = set(draw.numbers())
        in_draw = nums & DECAGONS
        cnt = len(in_draw)
        count_dist[cnt] = count_dist.get(cnt, 0) + 1
        for m in in_draw:
            freq[m] += 1
    ...
    freq_list = [
        {
            "number": m,
            "n": DECAGON_N[m],
            "formula": f"{DECAGON_N[m]}×(4×{DECAGON_N[m]}-3)",
            "count": freq[m],
            "pct": round(freq[m] / total * 100, 1),
        }
        for m in sorted(DECAGONS)
    ]
    ...
    return {
        "total": total,
        "decagon_count": len(DECAGONS),
        "decagon_list": sorted(DECAGONS),
        "avg": avg,
        "expected": expected,
        "diff": round(avg - expected, 3),
        "best_count": best_count,
...
def get_hendecagonal_analysis() -> dict | None:
    """SPEC-LOTTO-182: 십일각수(Hendecagonal Number) 포함 분포 분석.
    ...
    # H(n) = n(9n-7)/2; 1~45 범위 내 십일각수
    HENDECAGONS = {1, 11, 30}  # noqa: N806
    HENDECAGON_N = {1: 1, 11: 2, 30: 3}  # noqa: N806
    total = len(draws)
    expected = round(len(HENDECAGONS) / 45 * 6, 3)
    max_k = min(6, len(HENDECAGONS))

    count_dist: dict[int, int] = {k: 0 for k in range(max_k + 1)}  # noqa: C420
    freq: dict[int, int] = {m: 0 for m in HENDECAGONS}  # noqa: C420

    for draw in draws:
        nums = set(draw.numbers())
        in_draw = nums & HENDECAGONS
        cnt = len(in_draw)
        count_dist[cnt] = count_dist.get(cnt, 0) + 1
        for m in in_draw:
            freq[m] += 1
    ...
    freq_list = [
        {
            "number": m,
            "n": HENDECAGON_N[m],
            "formula": f"{HENDECAGON_N[m]}×(9×{HENDECAGON_N[m]}-7)÷2",
            "count": freq[m],
            "pct": round(freq[m] / total * 100, 1),
        }
        for m in sorted(HENDECAGONS)
    ]
    ...
    return {
        "total": total,
        "hendecagon_count": len(HENDECAGONS),
        "hendecagon_list": sorted(HENDECAGONS),
        "avg": avg,
        "expected": expected,
        "diff": round(avg - expected, 3),
        "best_count": best_count,
...
def get_dodecagonal_analysis() -> dict | None:
    """SPEC-LOTTO-183: 십이각수(Dodecagonal Number) 포함 분포 분석.
    ...
    # D(n) = n(5n-4); 1~45 범위 내 십이각수
    DODECAGONS = {1, 12, 33}  # noqa: N806
    DODECAGON_N = {1: 1, 12: 2, 33: 3}  # noqa: N806
    total = len(draws)
    expected = round(len(DODECAGONS) / 45 * 6, 3)
    max_k = min(6, len(DODECAGONS))

    count_dist: dict[int, int] = {k: 0 for k in range(max_k + 1)}  # noqa: C420
    freq: dict[int, int] = {m: 0 for m in DODECAGONS}  # noqa: C420

    for draw in draws:
        nums = set(draw.numbers())
        in_draw = nums & DODECAGONS
        cnt = len(in_draw)
        count_dist[cnt] = count_dist.get(cnt, 0) + 1
        for m in in_draw:
            freq[m] += 1
    ...
    freq_list = [
        {
            "number": m,
            "n": DODECAGON_N[m],
            "formula": f"{DODECAGON_N[m]}×(5×{DODECAGON_N[m]}-4)",
            "count": freq[m],
            "pct": round(freq[m] / total * 100, 1),
        }
        for m in sorted(DODECAGONS)
    ]
    ...
    return {
        "total": total,
        "dodecagon_count": len(DODECAGONS),
        "dodecagon_list": sorted(DODECAGONS),
        "avg": avg,
        "expected": expected,
        "diff": round(avg - expected, 3),
        "best_count": best_count,
...
def get_tetradecagonal_analysis() -> dict | None:
    """SPEC-LOTTO-185: 십사각수(Tetradecagonal Number) 포함 분포 분석.
    ...
    # F(n) = n(6n-5); 1~45 범위 내 십사각수
    TETRADECAGONS = {1, 14, 39}  # noqa: N806
    TETRADECAGON_N = {1: 1, 14: 2, 39: 3}  # noqa: N806
    total = len(draws)
    expected = round(len(TETRADECAGONS) / 45 * 6, 3)
    max_k = min(6, len(TETRADECAGONS))

    count_dist: dict[int, int] = {k: 0 for k in range(max_k + 1)}  # noqa: C420
    freq: dict[int, int] = {m: 0 for m in TETRADECAGONS}  # noqa: C420

    for draw in draws:
        nums = set(draw.numbers())
        in_draw = nums & TETRADECAGONS
        cnt = len(in_draw)
        count_dist[cnt] = count_dist.get(cnt, 0) + 1
        for m in in_draw:
            freq[m] += 1
    ...
    freq_list = [
        {
            "number": m,
            "n": TETRADECAGON_N[m],
            "formula": f"{TETRADECAGON_N[m]}×(6×{TETRADECAGON_N[m]}-5)",
            "count": freq[m],
            "pct": round(freq[m] / total * 100, 1),
        }
        for m in sorted(TETRADECAGONS)
    ]
    ...
    for draw in sorted(draws, key=lambda d: d.drwNo, reverse=True)[:20]:
        ...
        recent.append({
            "drwNo": draw.drwNo,
            "numbers": sorted(draw.numbers()),
            "tetradecagons": in_draw,
            "count": len(in_draw),
        })

    return {
        "total": total,
        "tetradecagon_count": len(TETRADECAGONS),
        "tetradecagon_list": sorted(TETRADECAGONS),
        "avg": avg,
        "expected": expected,
        "diff": round(avg - expected, 3),
        "best_count": best_count,
...
def get_pentadecagonal_analysis() -> dict | None:
    """SPEC-LOTTO-186: 십오각수(Pentadecagonal Number) 포함 분포 분석.
    ...
    # P(n) = (13n²-11n)/2; 1~45 범위 내 십오각수
    PENTADECAGONS = {1, 15, 42}  # noqa: N806
    PENTADECAGON_N = {1: 1, 15: 2, 42: 3}  # noqa: N806
    total = len(draws)
    expected = round(len(PENTADECAGONS) / 45 * 6, 3)
    max_k = min(6, len(PENTADECAGONS))

    count_dist: dict[int, int] = {k: 0 for k in range(max_k + 1)}  # noqa: C420
    freq: dict[int, int] = {m: 0 for m in PENTADECAGONS}  # noqa: C420

    for draw in draws:
        nums = set(draw.numbers())
        in_draw = nums & PENTADECAGONS
        cnt = len(in_draw)
        count_dist[cnt] = count_dist.get(cnt, 0) + 1
        for m in in_draw:
            freq[m] += 1
    ...
    freq_list = [
        {
            "number": m,
            "n": PENTADECAGON_N[m],
            "formula": f"(13×{PENTADECAGON_N[m]}²-11×{PENTADECAGON_N[m]})÷2",
            "count": freq[m],
            "pct": round(freq[m] / total * 100, 1),
        }
        for m in sorted(PENTADECAGONS)
    ]
    ...
    for draw in sorted(draws, key=lambda d: d.drwNo, reverse=True)[:20]:
        ...
        recent.append({
            "drwNo": draw.drwNo,
            "numbers": sorted(draw.numbers()),
            "pentadecagons": in_draw,
            "count": len(in_draw),
        })

    return {
        "total": total,
        "pentadecagon_count": len(PENTADECAGONS),
        "pentadecagon_list": sorted(PENTADECAGONS),
        "avg": avg,
        "expected": expected,
        "diff": round(avg - expected, 3),
        "best_count": best_count,
